-- Allow bet placement on either real normalized BSD odds or explicit
-- VirtualBook internal fallback odds. Legacy model-derived BSD odds remain
-- excluded at the database boundary.
      ...
      (
        ...
        where o.match_id = r.match_id
          ...
          and (
            (
              o.source = 'bsd'
              and o.pricing_method = 'bsd_market_normalized'
              and coalesce(o.is_model, false) = false
            )
            or (
              o.source = 'internal_model'
              and o.pricing_method = 'internal_model_fallback'
              and coalesce(o.is_model, false) = true
            )
          )
        order by
          case
...
revoke execute on function public.place_bet(numeric, jsonb, uuid)
from public, anon;
