...
function estimateExpectedGoals(event: UnknownRecord): {
  ...
  let drawStrength = 0.32;

  if (homeOdd && drawOdd && awayOdd) {
    [homeStrength, drawStrength, awayStrength] = normalizeProbabilities([
      1 / homeOdd,
      1 / drawOdd,
      1 / awayOdd,
    ]);
...
function buildModelOddsInputs(event: UnknownRecord): OddsInput[] {
  ...
  rows.push(input("dnb", "1", pHome / Math.max(pHome + pAway, 0.01), "model_dnb_1"));
  rows.push(input("dnb", "2", pAway / Math.max(pHome + pAway, 0.01), "model_dnb_2"));

  for (const line of [0.5, 1.5, 2.5]) {
    const key = String(line).replace(".", "_");
    rows.push(input(`home_ou_${key}`, "over", prob((h) => h > line), `model_home_ou_${key}_over`));
    rows.push(input(`home_ou_${key}`, "under", prob((h) => h < line), `model_home_ou_${key}_under`));
    rows.push(input(`away_ou_${key}`, "over", prob((_, a) => a > line), `model_away_ou_${key}_over`));
    rows.push(input(`away_ou_${key}`, "under", prob((_, a) => a < line), `model_away_ou_${key}_under`));
  }
  ...
  rows.push(input("exact_score", "other", 1 - listedExactScoreProb, "model_exact_score_other"));

  const htHomeXg = homeXg * 0.45;
  const htAwayXg = awayXg * 0.45;
  const stHomeXg = homeXg * 0.55;
  const stAwayXg = awayXg * 0.55;

  const halfMarkets = [
    { prefix: "ht", h: htHomeXg, a: htAwayXg },
    { prefix: "st", h: stHomeXg, a: stAwayXg },
  ];
  ...
  for (const half of halfMarkets) {
    ...
    for (let h = 0; h <= 5; h += 1) {
      for (let a = 0; a <= 5; a += 1) {
        halfMatrix.push({ h, a, p: poisson(half.h, h) * poisson(half.a, a) });
      }
    }

    const hp = (predicate: (h: number, a: number) => boolean) =>
      clamp(
        halfMatrix.filter((row) => predicate(row.h, row.a)).reduce((sum, row) => sum + row.p, 0),
        0.01,
        0.97
      );
    ...
    for (const line of [0.5, 1.5]) {
      rows.push(input(`${half.prefix}_ou_${String(line).replace(".", "_")}`, "over", hp((h, a) => h + a > line), `model_${half.prefix}_ou_${line}_over`));
      rows.push(input(`${half.prefix}_ou_${String(line).replace(".", "_")}`, "under", hp((h, a) => h + a < line), `model_${half.prefix}_ou_${line}_under`));
    }
